...
export class DigiStatusCardComponent implements AfterViewInit {
  ...
  private isParticipant = signal(false);
  private isGeneratingDialogue = signal(false);

  changedName = computed(() => {
    if (this.isThisDigimon()) return;
    return this.globalState.getDigimonById(
      this.globalState.selectedDigimonOnDetails()?.id!
    )?.nickName;
  });
  ...
  ngAfterViewInit(): void {
    ...
    this.dialogueService.currentLine$
      .pipe(takeUntilDestroyed(this.destroyRef), filter((line) => line !== undefined))
      .subscribe((line) => {
        const myId = this.digimon().id;
        if (!myId) {
    ...
    this.dialogueService.activeParticipants$
      .pipe(takeUntilDestroyed(this.destroyRef), filter((ids) => !!ids.length))
      .subscribe((ids) => {
        const myId = this.digimon()?.id;
        this.isParticipant.set(Boolean(myId) && ids.includes(myId));
        if (this.isGeneratingDialogue() && this.isParticipant()) {
          this.currentText.set('...');
          this.showBubble.set(true);
        }
    ...
    this.dialogueService.isStreaming$
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe((isStreaming) => {
        this.isGeneratingDialogue.set(isStreaming);
  ...
  onMouseEnter(): void {
    ...
    this.isDeletable.set(
      isPlayerDigimon &&
      hasMultipleDigimons &&
      !this.globalState.isBattleActive()
    );
  ...
  private animateFloatingBadge(text: string, type: BadgeType) {
    ...
    setTimeout(() => {
      ...
      requestAnimationFrame(() => {
        ...
        this.tlBadge = gsap
          .timeline({ onComplete: () => this.onBadgeAnimationDone() })
          .to(el, {
            opacity: 1,
            y: -8,
            scale: 1,
  ...
  private buildHitTimeline() {
    ...
    if (slash) {
      this.tlHit
        ...
        .add(() => {
          gsap.set(slash, { opacity: 0, scaleX: 0.2, xPercent: -30 });
        }, 0.28);
    }
    ...
    if (spark) {
      this.tlHit
        ...
        .add(() => {
          gsap.set(spark, { opacity: 0, scale: 0.6, rotate: 0 });
        }, 0.32);
    }
    ...
    this.tlHit
      .to(
        card,
        {
          boxShadow: '0 0 0.7rem rgba(255, 64, 64, 0.38)',
      ...
      .to(
        card,
        { boxShadow: 'none', filter: 'none', duration: 0.18, ease: 'power1.out' },
        0.18
      );

    this.tlHit.add(() => {
      (card.style as any).transition = '';
    }, '>');
  }
