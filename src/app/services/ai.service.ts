...
import { DialoguePayload, DialogueLine, OllamaStreamChunk } from '@core/interfaces/dialogue.interface';
...
export class AiService {
  ...
  async generateDialogue(sceneContext: string, digimons: Digimon[]): Promise<DialoguePayload> {
    ...
    try {
      ...
      if (this.wasCancelled) return this.wrapError('Dialogue generation cancelled.');
  ...
  async isModelInstalled(): Promise<boolean> {
    try {
      return await invoke<boolean>('ollama_has_model', { name: this.MODEL });
    } catch {
      return false;
    }
  }
...
${context}
  ...
  private extractBalancedJsonObjects(raw: string): { objects: string[]; remaining: string } {
    const result: string[] = [];
    let remaining = '';
    let depth = 0;
    let start = -1;
    ...
    return { objects: result, remaining };
  }

  private buildIdToNameMap(digimons: Digimon[]): Map<string, string> {
    const map = new Map<string, string>();
    for (const d of digimons) map.set(String(d.id || '').toLowerCase(), d.name);
    return map;
  }
